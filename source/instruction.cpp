...
namespace rvcpp {
  ...
  uint32_t Instruction::branch_target_address(uint32_t pc) const {
    ...
    return pc + static_cast<uint32_t>(static_cast<int32_t>(imm << 19) >> 19);
  }
  uint32_t Instruction::shifted_upimm() const { return raw & ~mask(12); }
  uint32_t Instruction::jump_target_address(uint32_t pc) const {
    ...
    return pc + static_cast<uint32_t>(static_cast<int32_t>(imm << 11) >> 11);
  }
}  // namespace rvcpp